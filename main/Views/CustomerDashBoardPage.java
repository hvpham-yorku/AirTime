...
public class CustomerDashBoardPage extends JPanel implements ActionListener {
	...
	private JButton transactionHistoryButton = new JButton("Transaction History");
	private JButton flightHistoryButton = new JButton("Flight History");
	...
	private void initialize() {
		 ...
		 * transactions.setBackground(Color.BLACK); transactions.addActionListener(e ->
		...
		viewCartButton.setBackground(Color.BLACK);
		...
		clearCartButton.setBackground(Color.BLACK);
	...
	private void searchByDepartureCity() {
		...
		try {
			...
			if (flights.isEmpty()) {
				JOptionPane.showMessageDialog(this, "No flights found for the departing city: " + city, "Error",
						JOptionPane.ERROR_MESSAGE);
			} else {
				updateFlightTable(flights);
			}
		} catch (Exception e) {
			JOptionPane.showMessageDialog(this, "Error searching for flights: " + e.getMessage(), "Error",
					JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private void searchByDestinationCity() {
		...
		try {
			...
			if (flights.isEmpty()) {
				JOptionPane.showMessageDialog(this, "No flights found for the destination city: " + city, "Error",
						JOptionPane.ERROR_MESSAGE);
			} else {
				updateFlightTable(flights);
			}
	...
	private void updateFlightTable(ArrayList<Flight> flights) {
		String[] columnNames = { "Flight ID", "Flight Number", "Departure City", "Destination City", "Departure Time",
				"Arrival Time", "Price", "Seats Available" };
		Object[][] tableData = new Object[flights.size()][columnNames.length];
	...
	private void filterByPriceRange() {
		...
		if (minPriceInput == null || minPriceInput.isEmpty())
			return;

		String maxPriceInput = JOptionPane.showInputDialog(this, "Enter the maximum price:");
		if (maxPriceInput == null || maxPriceInput.isEmpty())
			return;
		...
		try {
			...
			if (minPrice < 0) {
				JOptionPane.showMessageDialog(this, "Minimum price cannot be less than $0.", "Error",
						JOptionPane.ERROR_MESSAGE);
				return;
			}

			if (minPrice > maxPrice) {
				JOptionPane.showMessageDialog(this, "Minimum price cannot be greater than maximum price.", "Error",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			if (filteredFlights.isEmpty()) {
				JOptionPane.showMessageDialog(this, "No flights found within the given price range.", "Error",
						JOptionPane.ERROR_MESSAGE);
			} else {
			...
			JOptionPane.showMessageDialog(this, "Please enter valid numeric values for prices.", "Error",
					JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private void searchTravelTime() {
		...
		if (departureCity == null || destinationCity == null || departureCity.trim().isEmpty()
				|| destinationCity.trim().isEmpty()) {
			JOptionPane.showMessageDialog(this, "Please enter both departure and destination cities.", "Error",
					JOptionPane.ERROR_MESSAGE);
			return;
		}

		// Call the database method to find all flights between the cities
		ArrayList<Flight> flights = controller.getDatabase().getShortestTravelTimeFlights(departureCity.trim(),
		...
		if (flights != null && !flights.isEmpty()) {
			...
			JOptionPane.showMessageDialog(this, flights.size() + " flight(s) found and displayed.", "Info",
					JOptionPane.INFORMATION_MESSAGE);
		} else {
			JOptionPane.showMessageDialog(this, "No flights found between these cities.", "Info",
					JOptionPane.INFORMATION_MESSAGE);
		}
	}
	...
	private void showDateInputDialog() {
		...
		submitButton.addActionListener(e -> {
			...
			if (departureDate != null && departureDate.isBefore(LocalDate.now())) {
				JOptionPane.showMessageDialog(dateDialog, "Departure date cannot be in the past.", "Error",
						JOptionPane.ERROR_MESSAGE);
				return;
			}

			if (arrivalDate != null && arrivalDate.isBefore(departureDate)) {
				JOptionPane.showMessageDialog(dateDialog, "Arrival date cannot be before the departure date.", "Error",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
	...
	private void searchFlightsByDates(LocalDate departureDate, LocalDate arrivalDate) {
		...
		if (flights != null && !flights.isEmpty()) {
			...
			JOptionPane.showMessageDialog(this, "No flights found for the given dates.", "Info",
					JOptionPane.INFORMATION_MESSAGE);
		}
	}
	...
	private void transactionHistoryCheck() {
		...
		if (transactions.isEmpty()) {
			...
			JOptionPane.showMessageDialog(this, "Your transaction history is empty.", "Info",
					JOptionPane.INFORMATION_MESSAGE);
	...
	private void flightHistoryCheck() {
		...
		if (flights.isEmpty()) {
			JOptionPane.showMessageDialog(this, "Your flight history is empty.", "Info",
					JOptionPane.INFORMATION_MESSAGE);
		}
		...
		String[] columns = { "Flight ID", "Flight Number", "Departure City", "Destination City", "Departure Time",
				"Arrival Time", "Price" };
	...
	private void cancelFlight() {
		...
		if (currFlights.isEmpty()) {
			JOptionPane.showMessageDialog(this, "You do not have any flights to cancel.", "Info",
					JOptionPane.INFORMATION_MESSAGE);
		}
			...
			if (flight.getFlightID() == flightID) {
				...
				for (Booking booking : bookings) {
					...
					if (booking.getFlightID() == flightID) {
						...
						LocalDateTime date = LocalDateTime.parse(booked, formatter); // parsing the String date in the
																						// booking.
		...
		if (!exists) {
			JOptionPane.showMessageDialog(this, "The flight you are trying to cancel does not exist.", "Error",
					JOptionPane.ERROR_MESSAGE);
		}
	...
	private void processPayment() {
		...
		if (cart.isEmpty()) {
			JOptionPane.showMessageDialog(this, "Your cart is empty. Add flights to your cart before paying.", "Error",
					JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		for (Flight flight : cart) {
			...
			if (seatNumber == null || seatNumber.isEmpty()) {
				JOptionPane.showMessageDialog(this,
						"Seat number is required for flight " + flight.getFlightNumber() + ".", "Error",
						JOptionPane.ERROR_MESSAGE);
				allBookingsSuccessful = false;
				continue;
			}

			boolean travelInsurance = JOptionPane.showConfirmDialog(this,
					"Add travel insurance for flight " + flight.getFlightNumber() + "?", "Travel Insurance",
		...
		if (allBookingsSuccessful) {
			JOptionPane.showMessageDialog(this, "Payment successful, and all flights booked!", "Success",
					JOptionPane.INFORMATION_MESSAGE);
			currentUser.clearCart(); // Clear the cart after successful payment
		} else {
			JOptionPane.showMessageDialog(this, "Some bookings failed. Please try again.", "Error",
					JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private void addTravelInsurance() {
		...
		try {
			...
			if (booking.isTravelInsurance()) { // Ensure travel_insurance field is accessible
				JOptionPane.showMessageDialog(this, "Travel insurance has already been added for this booking.",
						"Error", JOptionPane.ERROR_MESSAGE);
				return;
			}

			// Update the booking to add travel insurance
			boolean success = controller.getDatabase().updateBooking(bookingID, 0, null, true // Set travelInsurance to
																								// true
			);

			if (success) {
				JOptionPane.showMessageDialog(this, "Travel insurance added successfully!", "Success",
						JOptionPane.INFORMATION_MESSAGE);
			} else {
				JOptionPane.showMessageDialog(this, "Failed to add travel insurance. Please try again.", "Error",
						JOptionPane.ERROR_MESSAGE);
			}
